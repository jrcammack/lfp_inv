...
export class CategorySelectorComponent implements OnInit {
  ...
  searchStr: string = "";
  currentProductContext: Product = new Product(
    null,
  ...
  constructor(private http: HttpClient, private router: Router) {
      ...
      .subscribe((rspdata) => {
        ...
        for (let row of data.rows) {
          this.allPartners.push(
            new Partner(row.partner_id, row.partner_name, row.partner_code)
          );
    ...
    http
      ...
      .subscribe((rspdata) => {
        let data: any = rspdata;
        for (let row of data.rows) {
          let currentShipmentIteration: Shipment = new Shipment(
            row.shipment_id,
            row.date_received,
            this.partnerIdToCodePairings.get(parseInt(row.partner_id))
          );
          this.allShipments.push(currentShipmentIteration);
          this.shipmentIdToShipmentPairings.set(
            row.shipment_id,
            currentShipmentIteration
          );
        }
      });
  }
  ...
  onFormSubmit(form: NgForm) {
    ...
    this.http
      ...
      .subscribe((rsp) => {
        ...
        this.shipmentContext = new Shipment(
          id,
          form.value.date,
          this.partnerIdToCodePairings.get(parseInt(form.value.partner))
        );
      });
  }

  onPreviousShipmentSelect(event: PointerEvent) {
    this.shipmentContext = this.shipmentIdToShipmentPairings.get(
      parseInt((<HTMLLinkElement>event.target).id)
    );
  }
    ...
    this.http
      ...
      .subscribe((rspdata) => {
        ...
        for (let row of data.rows) {
          ...
          if (
            this.allShipments.find(function (item) {
              return item.shipmentID == currentShipmentIteration.shipmentID;
            })
          ) {
      ...
      .subscribe((rsp) => {
        ...
        if (data.has_data) {
          if (
            ...
            data.result.rows[0].has_details == false
          ) {
            ...
            this.currentProductContext = new Product(
              detailsArr[0].product_sku,
              this.subcategories.find((value, index) => {
                return value.id == detailsArr[0].sub_category_id;
              }),
              detailsArr[0].shipment_id,
              detailsArr[0].chargerback_id,
              detailsArr[0].status_id,
              detailsArr[0].has_details
            );
          } else {
            let detailsArr: any = data.result.rows;
            if (detailsArr == undefined || detailsArr == null) {
            ...
            for (let prodDetail of data.result.rows) {
              this.currentProductContext.detailsMap.set(
                prodDetail.detail_name,
                prodDetail.detail_value
              );
            }
          }
        } else {
