...
export class BulkEntryBodyComponent {
  ...
  onFormSubmit(formObj: NgForm) {
    ...
    // error checking
    if (
      this.shipmentContext.shipmentID == 0 ||
      this.subCategoryContext.id == 0 ||
      this.itemCount <= 0
    ) {
    ...
    let dupeArr: Array<string> = skuArray.filter(
    ...
    if (dupeArr.length > 0) {
      ...
      let bulkReq = new BulkPostRequest(
        this.shipmentContext,
        this.subCategoryContext,
        skuArray
      );
      this.http
        .post(
          "https://lfp-inv.herokuapp.com/bulkEntry" /*'http://localhost:4201/bulkEntry'*/,
          bulkReq
        )
        .subscribe((rsp) => {
          console.log(rsp);
